...
public class EmployeesModel {
    ...
    private EmployeesPresenter.GetEmployeesCallback callback;

    private void requestJson(JsonConnect.LoadJsonCallback callback) {
        JsonConnect strConnect = new JsonConnect(callback);
        strConnect.execute();
    }

    public void getContainerEmployees(EmployeesPresenter.GetEmployeesCallback callback){
        this.callback = callback;
        requestJson(new JsonConnect.LoadJsonCallback() {
            @Override
